...
const AdminDashboard = () => {
  ...
  const toggleTableStatus = (id) => {
    setTables(tables.map(t => {
      if (t.id !== id) return t;
      const nextStatus = t.status === 'available' ? 'occupied' : t.status === 'occupied' ? 'action' : t.status === 'action' ? 'available' : 'available';
      return { ...t, status: nextStatus };
    }));
  };

  const handleAction = (actionType) => {
    if (actionType === 'Call Next Guest') {
      alert('📣 Calling next guest: Sarah J. (Party of 4)');
    } else if (actionType === 'Add Walk-in') {
      alert('Navigating to Live Queue to add walk-in guest...');
    } else if (actionType === 'Mark Table Ready') {
      alert('Table marked as clean and ready for seating!');
    }
  };
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }}>
        ...
        <div className="grid grid-cols-12 gap-6">
          ...
          <div className="col-span-12 lg:col-span-5 space-y-4">
            ...
            <StatCard label="Tables Occupied" value={`${tables.filter(t => t.status !== 'available').length}/${tables.length}`} sub={`${Math.round((tables.filter(t => t.status !== 'available').length / tables.length) * 100)}% capacity`} color="text-[#171d16]" icon={<LayoutGrid className="w-5 h-5 text-[#006e1c]" />} progress={Math.round((tables.filter(t => t.status !== 'available').length / tables.length) * 100)} />
          ...
          <div className="col-span-12 lg:col-span-7 space-y-5">
            ...
            <div className="bg-white rounded-xl border border-[#becab9] p-5 shadow-sm">
              ...
              <div className="flex items-end gap-3 h-32">
                {bars.map(b => (
                  <div key={b.label} className="flex flex-col items-center gap-2 flex-1 hover:-translate-y-1 transition-transform cursor-pointer">
                    <div className="w-full rounded-t-md transition-all" style={{ height: `${b.h}%`, background: b.active ? '#006e1c' : '#dee4d9' }} title={`${b.h}% Occupied`} />
